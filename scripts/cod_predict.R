## Estimate age-o cod abundance from seines for each year
...
cod_recr_1_formula <-  bf(cod ~ s(julian, k = 3) + (1 | bay_fac) + (year_fac),
                    zi ~ s(julian, k = 3) + (1 | bay_fac) + (year_fac))

cod_recr_2_formula <-  bf(cod ~ s(julian, k = 3) + (1 | bay_fac/site_fac) + (year_fac),
...
## Show default priors
get_prior(cod_recr_1_formula, cod.data, family = zinb)
...
## fit: zero-inflated --------------------------------------
cod_recr_1_zinb <- brm(cod_recr_1_formula,
                   data = cod.data,
                   prior = priors_zinb,
                   family = zinb,
                   seed = 123,
...
saveRDS(cod_recr_1_zinb, file = "output/cod_recr_1_zinb.rds")
cod_recr_1_zinb  <- add_criterion(cod_recr_1_zinb, c("loo", "bayes_R2"), moment_match = TRUE)

cod_recr_1_zinb <- readRDS("./output/cod_recr_1_zinb.rds")
check_hmc_diagnostics(cod_recr_1_zinb$fit)
neff_lowest(cod_recr_1_zinb$fit)
rhat_highest(cod_recr_1_zinb$fit)
summary(cod_recr_1_zinb)
bayes_R2(cod_recr_1_zinb)
plot(cod_recr_1_zinb$criteria$loo, "k")
plot(conditional_smooths(cod_recr_1_zinb), ask = FALSE)
plot(cod_recr_1_zinb, ask = FALSE)
y <- cod.data$cod
yrep_cod_recr_1_zinb  <- fitted(cod_recr_1_zinb, scale = "response", summary = FALSE)
ppc_dens_overlay(y = y, yrep = yrep_cod_recr_1_zinb[sample(nrow(yrep_cod_recr_1_zinb), 25), ]) +
  xlim(0, 500) +
  ggtitle("cod_recr_1_zinb")
pdf("./figs/trace_cod_recr_1_zinb.pdf", width = 6, height = 4)
trace_plot(cod_recr_1_zinb$fit)
dev.off()

cod_recr_2_zinb <- brm(cod_recr_2_formula,
                   data = cod.data,
                   prior = priors_zinb,
                   family = zinb,
                   seed = 12345,
...
cod_recr_2_zinb  <- add_criterion(cod_recr_2_zinb, c("loo", "bayes_R2"), moment_match = TRUE)
saveRDS(cod_recr_2_zinb, file = "output/cod_recr_2_zinb.rds")

cod_recr_2_zinb <- readRDS("./output/cod_recr_2_zinb.rds")
check_hmc_diagnostics(cod_recr_2_zinb$fit)
neff_lowest(cod_recr_2_zinb$fit)
rhat_highest(cod_recr_2_zinb$fit)
summary(cod_recr_2_zinb)
bayes_R2(cod_recr_2_zinb)
plot(cod_recr_2_zinb$criteria$loo, "k")
plot(conditional_smooths(cod_recr_2_zinb), ask = FALSE)
y <- cod.data$cod
yrep_cod_recr_2_zinb  <- fitted(cod_recr_2_zinb, scale = "response", summary = FALSE)
ppc_dens_overlay(y = y, yrep = yrep_cod_recr_2_zinb[sample(nrow(yrep_cod_recr_2_zinb), 25), ]) +
  xlim(0, 500) +
  ggtitle("cod_recr_2_zinb")
pdf("./figs/trace_cod_recr_2_zinb.pdf", width = 6, height = 4)
trace_plot(cod_recr_2_zinb$fit)
dev.off()
...
loo(cod_recr_1_zinb, cod_recr_2_zinb)
